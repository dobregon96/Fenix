...
class weapon_amerk : ScriptBasePlayerWeaponEntity
{
	...
	{
		self.Precache();
		g_EntityFuncs.SetModel( self, self.GetW_Model( "models/ww2projekt/amerk/w_amerk.mdl") );
		self.m_iClip			= -1;
	...
	void Precache()
	{
		...
		g_Game.PrecacheModel( "models/ww2projekt/amerk/w_amerk.mdl" );
		g_Game.PrecacheModel( "models/ww2projekt/amerk/v_amerk.mdl" );
		g_Game.PrecacheModel( "models/ww2projekt/amerk/p_amerk.mdl" );
		...
		g_Game.PrecacheGeneric( "sprites/" + "ww2projekt/weapon_amerk.txt" );
	}
	...
	bool Deploy()
	{
		bool bResult;
		{
			bResult = self.DefaultDeploy( self.GetV_Model( "models/ww2projekt/amerk/v_amerk.mdl" ), self.GetP_Model( "models/ww2projekt/amerk/p_amerk.mdl" ), BAYO_DRAW, "crowbar" );
	...
	bool AddToPlayer( CBasePlayer@ pPlayer )
	{
		...
		{
			@m_pPlayer = pPlayer;
			NetworkMessage allies11( MSG_ONE, NetworkMessages::WeapPickup, pPlayer.edict() );
				allies11.WriteLong( g_ItemRegistry.GetIdForName("weapon_amerk") );
			allies11.End();
			return true;
		}
	...
	void Holster( int skiplocal /* = 0 */ )
	{
		...
		m_pPlayer.m_flNextAttack = g_Engine.time + 0.45; 
		SetThink( null );
		m_pPlayer.pev.viewmodel = "models/ww2projekt/amerk/v_amerk.mdl";
	}
	...
	bool Swing( int fFirst )
	{
		...
		if ( tr.flFraction >= 1.0 )
		{
			if( fFirst != 0 )
			{
				...
				{
				case 0:
					self.SendWeaponAnim( BAYO_SLASH2 ); break;
				case 1:
					self.SendWeaponAnim( BAYO_SLASH1 ); break;
				}
				self.m_flNextPrimaryAttack = self.m_flNextSecondaryAttack = g_Engine.time + 0.45;
				// play wiff or swish sound
				switch( Math.RandomLong( 0, 1 ) )
				{
					case 0: g_SoundSystem.EmitSoundDyn( m_pPlayer.edict(), CHAN_WEAPON, "weapons/ww2projekt/knifeswing.wav", 1, ATTN_NORM, 0, 94 + Math.RandomLong( 0,0xF ) ); break;
					case 1: g_SoundSystem.EmitSoundDyn( m_pPlayer.edict(), CHAN_WEAPON, "weapons/ww2projekt/knifeswing2.wav", 1, ATTN_NORM, 0, 94 + Math.RandomLong( 0,0xF ) ); break;
				}
...
void RegisterBAYO()
{
	g_CustomEntityFuncs.RegisterCustomEntity( GetBAYOName(), GetBAYOName() );
	g_ItemRegistry.RegisterWeapon( GetBAYOName(), "ww2projekt" );
}