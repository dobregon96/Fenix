...
const int K98K_DEFAULT_GIVE		= 5 * 2;
...
class weapon_kar98k : ScriptBasePlayerWeaponEntity
{
	...
	void Precache()
	{
		...
		g_Game.PrecacheModel( "models/ww2projekt/kar98k/v_98k.mdl" );
	...
	bool AddToPlayer( CBasePlayer@ pPlayer )
	{
		if( BaseClass.AddToPlayer( pPlayer ) )
		{
			...
			NetworkMessage axis7( MSG_ONE, NetworkMessages::WeapPickup, pPlayer.edict() );
				axis7.WriteLong( g_ItemRegistry.GetIdForName("weapon_kar98k") );
			axis7.End();
			return true;
		}
	...
	bool Deploy()
	{
		bool bResult;
		{
			bResult = self.DefaultDeploy( self.GetV_Model( "models/ww2projekt/kar98k/v_98k.mdl" ), self.GetP_Model( "models/ww2projekt/kar98k/p_k98.mdl" ), K98K_DRAW, "sniper" );
	...
	void PrimaryAttack()
	{
		...
		self.m_flNextPrimaryAttack = self.m_flNextSecondaryAttack = WeaponTimeBase() + 1.59;
		...
		m_pPlayer.FireBullets( 1, vecSrc, vecAiming, g_vecZero, 8192.0f, BULLET_PLAYER_CUSTOMDAMAGE, 2, m_iBulletDamage );
	...
	void EjectThink()
	{
		...
		GetDefaultShellInfo( m_pPlayer, vecShellVelocity, vecShellOrigin, 19, 6, -7, false, false );
	...
	void Reload()
	{
		if( self.m_iClip == K98K_MAX_CLIP || m_pPlayer.m_rgAmmo( self.m_iPrimaryAmmoType ) <= 0 )
			return;

		self.DefaultReload( K98K_MAX_CLIP, K98K_RELOAD, 3.15, 0 );
		BaseClass.Reload();
	}
