...
const int ENFIELD_DEFAULT_GIVE	= 10 * 2;
...
class weapon_enfield : ScriptBasePlayerWeaponEntity
{
	...
	bool AddToPlayer( CBasePlayer@ pPlayer )
	{
		if( BaseClass.AddToPlayer ( pPlayer ) )
		{
			...
			NetworkMessage british1( MSG_ONE, NetworkMessages::WeapPickup, pPlayer.edict() );
				british1.WriteLong( g_ItemRegistry.GetIdForName("weapon_enfield") );
			british1.End();
			return true;
		}
	...
	void PrimaryAttack()
	{
		...
		g_SoundSystem.EmitSoundDyn( m_pPlayer.edict(), CHAN_WEAPON, "weapons/ww2projekt/enfield_shoot1.wav", 0.8, ATTN_NORM, 0, PITCH_NORM );
	...
	void Reload()
	{
		if( self.m_iClip == ENFIELD_MAX_CLIP || m_pPlayer.m_rgAmmo( self.m_iPrimaryAmmoType ) <= 0 )
			return;

		if( self.m_iClip >= 5 || m_pPlayer.m_rgAmmo( self.m_iPrimaryAmmoType ) <= 5 )
		{
			self.DefaultReload( ENFIELD_MAX_CLIP, ENFIELD_RELOAD, 3.68f, 0 );
		}
		else if( self.m_iClip < 5 )
		{
			self.DefaultReload( ENFIELD_MAX_CLIP, ENFIELD_RELOAD_LONG, 5.48f, 0 );
		}
