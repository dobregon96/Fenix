/*
* Vector-T modification of point_checkpoint by SC team
* This script only modify the trigger options, in which checkpoint can now be triggered via trigger, button, etc..
* Known bugs: Rendermode will be reset to 255 when Reusable flags is set during respawn
* Do not use this one for mapping use, this one disable the exit sprite effect without mapping properties
...
class point_checkpoint : ScriptBaseAnimating
{
	...
	void Precache()
	{
		...
		g_SoundSystem.PrecacheSound( "bmwar/checkpoint_survivalsting.wav" );
	...
	{
		if( !IsEnabled() || IsActivated() || !pActivator.IsPlayer() )
			return;
		
		// Set activated
		self.pev.frags = 1.0f;
		
		g_SoundSystem.EmitSound( self.edict(), CHAN_STATIC, "bmwar/checkpoint_survivalsting.wav", 1.0f, ATTN_NONE );
