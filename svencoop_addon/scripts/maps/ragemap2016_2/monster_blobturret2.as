...
const float TURRET2_FIRERATE = 0.5f;
...
const float TURRET_MAXDIST	= 1024;//16384
...
class CBlobTurret2 : ScriptBaseMonsterEntity
{
	int		m_iDeployHeight;
	int 	m_iMinPitch;
	...
	void Spawn()
	{
		...
		g_EntityFuncs.DispatchKeyValue( self.edict(), "displayname", "Blob Turret" );
		self.pev.body		= 1;
...
*/
	...
	void Materialize()
	{
		...
		self.pev.nextthink = g_Engine.time + 0.02f;
	}
	...
	void ActiveThink()
	{
		...
		if( !fEnemyVisible || flDistToEnemy > TURRET_MAXDIST )
		{
			if( m_flLastSight <= 0.0 )
				m_flLastSight = g_Engine.time;
			else
		...
		m_vecCurAngles.x = -m_vecCurAngles.x;

		Vector vecLOS = vecDirToEnemy;
		vecLOS = vecLOS.Normalize();

		// Is the Gun looking at the target
		if( DotProduct(vecLOS, g_Engine.v_forward) <= 0.996f ) // 5 degree slop
			fAttack = false;
		else
			fAttack = true;
	...
	CBaseEntity@ BestVisibleEnemy()
	{
		...
		while( ( @pReturn = g_EntityFuncs.FindEntityInSphere( pReturn, self.pev.origin, TURRET_MAXDIST, "monster_scientist", "classname" ) ) !is null )
		{
			if( pReturn.IsAlive() && pReturn.FVisible( self, false ) )
				return pReturn;
		}
		return pReturn;
	}
	...
	void Shoot( Vector& in vecSrc, Vector& in vecDirToEnemy )
	{
		if( g_Engine.time >= m_flShootTime )
		{
			...
			ShootBiomass( self.pev, vecSrc, g_Engine.v_forward * 1500, 50 );
			SetTurretAnim(TURRET_ANIM_FIRE_MISSILE);
			m_flShootTime = g_Engine.time + TURRET2_FIRERATE;
		}
	}
}

void RegisterBlobTurret2()
{
	g_CustomEntityFuncs.RegisterCustomEntity( "CBlobTurret2", "monster_blobturret2" );
}