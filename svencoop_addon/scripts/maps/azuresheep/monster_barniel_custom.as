/* 
* Custom Barniel NPC Entity- this was done to get around Barniel using Barney voices
* Call barnielCustom::Register() to register this entity.
* Entity classname: monster_barniel_custom
*/

namespace barnielCustom
{

const int barniel_AE_DRAW			= 2;
const int barniel_AE_SHOOT			= 3;
const int barniel_AE_HOLSTER		= 4;
const int barniel_BODY_GUNHOLSTERED	= 0;
const int barniel_BODY_GUNDRAWN		= 1;
const int barniel_BODY_GUNGONE		= 2;

class CMonsterbarnielCustom : ScriptBaseMonsterEntity
{
	...
	CMonsterbarnielCustom()
	{
		@this.m_Schedules = @monster_barniel_schedules;
	}
	...
	void HandleAnimEvent( MonsterEvent@ pEvent )
	{
		...
		case barniel_AE_DRAW:
			...
			self.pev.body = barniel_BODY_GUNDRAWN;
			m_fGunDrawn = true;
			break;

		case barniel_AE_HOLSTER:
			// change bodygroup to replace gun in holster
			self.pev.body = barniel_BODY_GUNHOLSTERED;
			m_fGunDrawn = false;
			break;
	...
	void Precache()
	{
		...
		if( string( self.pev.model ).IsEmpty() )
		{
			...
			if( self.IsPlayerAlly() )
				g_Game.PrecacheModel("models/azuresheep/barniel.mdl");
			else
				g_Game.PrecacheModel("models/azuresheep/barniel.mdl");
		}

		//g_SoundSystem.PrecacheSound("barniel/ba_attack1.wav");
		g_SoundSystem.PrecacheSound("azuresheep/barniel/bn_becarefull.wav");
		g_SoundSystem.PrecacheSound("azuresheep/barniel/bn_carefull.wav");
		g_SoundSystem.PrecacheSound("azuresheep/barniel/bn_pain1.wav");
		g_SoundSystem.PrecacheSound("azuresheep/barniel/bn_pain2.wav");
		g_SoundSystem.PrecacheSound("azuresheep/barniel/bn_die1.wav");
	...
	{
		if( self.IsPlayerAlly() )
			g_EntityFuncs.SetModel( self, "models/azuresheep/barniel.mdl" );
		else
			g_EntityFuncs.SetModel( self, "models/azuresheep/barniel.mdl" );
	}
	...
	void FearScream()
	{
		if( m_flNextFearScream < g_Engine.time )
		{
			...
			{
			case 0: g_SoundSystem.EmitSoundDyn( self.edict(), CHAN_VOICE, "azuresheep/barniel/bn_becarefull.wav", 1, ATTN_NORM, 0, PITCH_NORM); break;
			case 1: g_SoundSystem.EmitSoundDyn( self.edict(), CHAN_VOICE, "azuresheep/barniel/bn_carefull.wav", 1, ATTN_NORM, 0, PITCH_NORM); break;
			case 2: g_SoundSystem.EmitSoundDyn( self.edict(), CHAN_VOICE, "azuresheep/barniel/bn_carefull.wav", 1, ATTN_NORM, 0, PITCH_NORM); break;
			}
	...
	void PainSound()
	{
		...
		switch (Math.RandomLong(0,2))
		{
		case 0: g_SoundSystem.EmitSoundDyn( self.edict(), CHAN_VOICE, "azuresheep/barniel/bn_pain1.wav", 1, ATTN_NORM, 0, PITCH_NORM); break;
		case 1: g_SoundSystem.EmitSoundDyn( self.edict(), CHAN_VOICE, "azuresheep/barniel/bn_pain2.wav", 1, ATTN_NORM, 0, PITCH_NORM); break;
		case 2: g_SoundSystem.EmitSoundDyn( self.edict(), CHAN_VOICE, "azuresheep/barniel/bn_pain2.wav", 1, ATTN_NORM, 0, PITCH_NORM); break;
		}
	}
	...
	void DeathSound()
	{
		switch (Math.RandomLong(0,2))
		{
		case 0: g_SoundSystem.EmitSoundDyn( self.edict(), CHAN_VOICE, "azuresheep/barniel/bn_die1.wav", 1, ATTN_NORM, 0, PITCH_NORM); break;
		case 1: g_SoundSystem.EmitSoundDyn( self.edict(), CHAN_VOICE, "azuresheep/barniel/bn_die1.wav", 1, ATTN_NORM, 0, PITCH_NORM); break;
		case 2: g_SoundSystem.EmitSoundDyn( self.edict(), CHAN_VOICE, "azuresheep/barniel/bn_die1.wav", 1, ATTN_NORM, 0, PITCH_NORM); break;
		}
	}
	...
	{		
		...
		case SCHED_ARM_WEAPON:
			if ( self.m_hEnemy.IsValid() )
				return slbarnielEnemyDraw;// face enemy, then draw.
			break;
	...
	Schedule@ GetSchedule()
	{
		...
		if ( self.HasConditions( bits_COND_ENEMY_DEAD ) )
			g_SoundSystem.PlaySentenceGroup( self.edict(), "BN_KILL", 1.0, ATTN_NORM, 0, PITCH_NORM );
		...
		case MONSTERSTATE_COMBAT:
			{
				...
				if ( self.HasConditions ( bits_COND_NO_AMMO_LOADED ) )
					return self.GetScheduleOfType ( SCHED_barniel_RELOAD );
			}
			break;
	...
	void FollowerUse( CBaseEntity@ pActivator, CBaseEntity@ pCaller, USE_TYPE useType, float flValue )
	{
		...
		{
			g_SoundSystem.PlaySentenceGroup( self.edict(), "BN_OK", 1.0, ATTN_NORM, 0, PITCH_NORM );
		}
		else
			g_SoundSystem.PlaySentenceGroup( self.edict(), "BN_WAIT", 1.0, ATTN_NORM, 0, PITCH_NORM );
	}
}

array<ScriptSchedule@>@ monster_barniel_schedules;
...
ScriptSchedule slBaReloadQuick(
	...
	bits_COND_HEAR_SOUND,
	bits_SOUND_DANGER,
	"barniel Reload Quick");
		
ScriptSchedule slbarnielEnemyDraw( 0, 0, "barniel Enemy Draw" );
...
void InitSchedules()
{
	...
	slbarnielEnemyDraw.AddTask( ScriptTask(TASK_STOP_MOVING) );
	slbarnielEnemyDraw.AddTask( ScriptTask(TASK_FACE_ENEMY) );
	slbarnielEnemyDraw.AddTask( ScriptTask(TASK_PLAY_SEQUENCE_FACE_ENEMY, float(ACT_ARM)) );
	...
	array<ScriptSchedule@> scheds = {slBaFollow, slbarnielEnemyDraw, slBaFaceTarget, slIdleBaStand, slBaReload, slBaReloadQuick};
	
	@monster_barniel_schedules = @scheds;
}

enum monsterScheds
{
	SCHED_barniel_RELOAD = LAST_COMMON_SCHEDULE + 1,
}

void Register()
{
	InitSchedules();
	g_CustomEntityFuncs.RegisterCustomEntity( "barnielCustom::CMonsterbarnielCustom", "monster_barniel_custom" );
}
