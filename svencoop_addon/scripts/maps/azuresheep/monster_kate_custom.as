/* 
* Custom Kate NPC Entity- this was done to get around Kate using Barney voices
* Call kateCustom::Register() to register this entity.
* Entity classname: monster_kate_custom
*/

namespace kateCustom
{

const int kate_AE_DRAW			 = 2;
const int kate_AE_SHOOT			 = 3;
const int kate_AE_HOLSTER		 = 4;
const int kate_BODY_GUNHOLSTERED = 0;
const int kate_BODY_GUNDRAWN	 = 1;
const int kate_BODY_GUNGONE		 = 2;

class CMonsterkateCustom : ScriptBaseMonsterEntity
{
	...
	CMonsterkateCustom()
	{
		@this.m_Schedules = @monster_kate_schedules;
	}
	...
	void HandleAnimEvent( MonsterEvent@ pEvent )
	{
		...
		case kate_AE_DRAW:
			...
			self.pev.body = kate_BODY_GUNDRAWN;
			m_fGunDrawn = true;
			break;

		case kate_AE_HOLSTER:
			// change bodygroup to replace gun in holster
			self.pev.body = kate_BODY_GUNHOLSTERED;
			m_fGunDrawn = false;
			break;
	...
	void Precache()
	{
		...
		if( string( self.pev.model ).IsEmpty() )
		{
			...
			if( self.IsPlayerAlly() )
				g_Game.PrecacheModel("models/azuresheep/kate.mdl");
			else
				g_Game.PrecacheModel("models/azuresheep/kate.mdl");
		}

		//g_SoundSystem.PrecacheSound("kate/ka_attack1.wav");
		g_SoundSystem.PrecacheSound("azuresheep/kate/becarefull.wav");
	...
	{
		if( self.IsPlayerAlly() )
			g_EntityFuncs.SetModel( self, "models/azuresheep/kate.mdl" );
		else
			g_EntityFuncs.SetModel( self, "models/azuresheep/kate.mdl" );
	}
	...
	void FearScream()
	{
		if( m_flNextFearScream < g_Engine.time )
		{
			...
			{
			case 0: g_SoundSystem.EmitSoundDyn( self.edict(), CHAN_VOICE, "azuresheep/kate/becarefull.wav", 1, ATTN_NORM, 0, PITCH_NORM); break;
			case 1: g_SoundSystem.EmitSoundDyn( self.edict(), CHAN_VOICE, "azuresheep/kate/becarefull.wav", 1, ATTN_NORM, 0, PITCH_NORM); break;
			case 2: g_SoundSystem.EmitSoundDyn( self.edict(), CHAN_VOICE, "azuresheep/kate/becarefull.wav", 1, ATTN_NORM, 0, PITCH_NORM); break;
			}
	...
	{		
		...
		case SCHED_ARM_WEAPON:
			if ( self.m_hEnemy.IsValid() )
				return slkateEnemyDraw;// face enemy, then draw.
			break;
	...
	Schedule@ GetSchedule()
	{
		...
		if ( self.HasConditions( bits_COND_ENEMY_DEAD ) )
			g_SoundSystem.PlaySentenceGroup( self.edict(), "KA_KILL", 1.0, ATTN_NORM, 0, PITCH_NORM );
		...
		case MONSTERSTATE_COMBAT:
			{
				...
				if ( self.HasConditions ( bits_COND_NO_AMMO_LOADED ) )
					return self.GetScheduleOfType ( SCHED_kate_RELOAD );
			}
			break;
	...
	void FollowerUse( CBaseEntity@ pActivator, CBaseEntity@ pCaller, USE_TYPE useType, float flValue )
	{
		...
		{
			g_SoundSystem.PlaySentenceGroup( self.edict(), "KA_OK", 1.0, ATTN_NORM, 0, PITCH_NORM );
		}
		else
			g_SoundSystem.PlaySentenceGroup( self.edict(), "KA_WAIT", 1.0, ATTN_NORM, 0, PITCH_NORM );
	}
}

array<ScriptSchedule@>@ monster_kate_schedules;
...
ScriptSchedule slkateEnemyDraw( 0, 0, "kate Enemy Draw" );
...
void InitSchedules()
{
	...
	slkateEnemyDraw.AddTask( ScriptTask(TASK_STOP_MOVING) );
	slkateEnemyDraw.AddTask( ScriptTask(TASK_FACE_ENEMY) );
	slkateEnemyDraw.AddTask( ScriptTask(TASK_PLAY_SEQUENCE_FACE_ENEMY, float(ACT_ARM)) );
	...
	array<ScriptSchedule@> scheds = {slBaFollow, slkateEnemyDraw, slBaFaceTarget, slIdleBaStand, slBaReload, slBaReloadQuick};
	
	@monster_kate_schedules = @scheds;
}

enum monsterScheds
{
	SCHED_kate_RELOAD = LAST_COMMON_SCHEDULE + 1,
}

void Register()
{
	InitSchedules();
	g_CustomEntityFuncs.RegisterCustomEntity( "kateCustom::CMonsterkateCustom", "monster_kate_custom" );
}
